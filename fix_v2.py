...
# Strategy: find start of hotlist, then find the NEXT major function definition (not a comment)
start = content.find('// ── Hotlist')
if start == -1:
    print('FATAL: no hotlist marker'); sys.exit(1)

# Find all function declarations after the hotlist section
# Look for "function clearInput" or "function parseRawTopics" etc - these are the next real functions
candidates = [
    '\nfunction clearInput',
    '\nfunction parseRawTopics',
...
for cand in candidates:
    pos = content.find(cand, start + 10)
    if pos != -1 and pos < end:
        end = pos
        print('Found end candidate: %s at char %d' % (cand.strip(), pos))

print('Will replace chars %d to %d (%d chars)' % (start, end, end - start))

# Show what we're removing (first 200 chars)
print('Removing:')
...
function renderHotlistData(key) {
  ...
  html += '\U0001f517 \u524d\u5f80\u4eca\u65e5\u70ed\u699c &rarr;</a></div>';
  el.innerHTML = html;
}
  ...
  try {
    ...
    if (j.success && j.data) {
      ...
      for (var k=0;k<keys.length;k++) {
        ...
        if (kd && kd.data) STATIC_HOTLIST[keys[k]] = {name:kd.name||keys[k],icon:kd.icon||'',data:kd.data};
      }
      if (srcEl) srcEl.textContent = '\u5b9e\u65f6 ('+new Date().toLocaleTimeString('zh-CN')+')';
      renderHotlistData(hotlistCurrentPlatform);
      return;
    }
    throw new Error('N/A');
  } catch(e) {
    if (srcEl) srcEl.textContent = '\u5185\u7f6e\u6570\u636e';
    renderHotlistData(hotlistCurrentPlatform);
  }
}
...
bad = re.findall(r'rank:\d+:title:', content)
print('Bad patterns remaining:', len(bad))
